...
public class WiimoteHandler
{
    ...
    public void HandleEvent(Wiimote wm)
    {
        DateTime currentTime = DateTime.Now;
        string timestamp = currentTime.ToString("HH:mm:ss");
        Console.WriteLine($"\n\n--- EVENT [id {wm.Unid}] ---");
        WBB1 = new StreamWriter("WiiMote1.txt");
        WBB2 = new StreamWriter("WiiMote2.txt");
        WBB3 = new StreamWriter("WiiMote1_All.txt", true);
        WBB4 = new StreamWriter("WiiMote2_All.txt", true);

        if (wm.Exp.Type == Wiimote.ExpansionType.WiiBalanceBoard)
        {
            Wiimote.WiiBalanceBoardData wb = wm.Expansion as Wiimote.WiiBalanceBoardData;
            float total = wb.TopLeft + wb.TopRight + wb.BottomLeft + wb.BottomRight;
            float x = (((wb.TopRight + wb.BottomRight) - (wb.TopLeft + wb.BottomLeft)) / total) * 26.05f;
            float y = (((wb.BottomRight + wb.BottomLeft) - (wb.TopRight + wb.TopLeft)) / total) * 16.75f;

            if (wm.Unid == 1)
            {
                WBB1.WriteLine($"{wb.TopLeft}\t{wb.TopRight}\t{wb.BottomLeft}\t{wb.BottomRight}\t{x}\t{y}\t{total}\t{timestamp}");
                WBB3.WriteLine($"{wb.TopLeft}\t{wb.TopRight}\t{wb.BottomLeft}\t{wb.BottomRight}\t{x}\t{y}\t{total}\t{timestamp}");
            }
            else if (wm.Unid == 2)
            {
                WBB2.WriteLine($"{wb.TopLeft}\t{wb.TopRight}\t{wb.BottomLeft}\t{wb.BottomRight}\t{x}\t{y}\t{total}\t{timestamp}");
                WBB4.WriteLine($"{wb.TopLeft}\t{wb.TopRight}\t{wb.BottomLeft}\t{wb.BottomRight}\t{x}\t{y}\t{total}\t{timestamp}");
            }
        }
...
public class Wiimote
{
    ...
    public class WiiBalanceBoardData : ExpansionData
    {
        ...
        private void button_RecordingToggle_Click(object sender, EventArgs e)
        {
            ...
            {
                // Start the recording.
                string fileName1 = DateTime.Now.ToString("yyyy-MM-dd-T-HH-mm-ss"_1) + ".csv";
                // First write the file header, only then set the variable.
                string header = "PC timestamp;Center of mass (X);Center of mass (Y);Top left (kg);Top right (kg);Bottom left (kg);Bottom right (kg)";
                File.WriteAllLines(fileName1, new string[] { header });
                recordingFile1 = fileName1;
                // Update the UI:
                button_RecordingToggle.Text = "Stop";
                groupBox_Recording.BackColor = System.Drawing.Color.Red;
                label_RecordingFile.Text = "Output file:\r\n" + recordingFile1;
                string data = DateTime.Now.ToString("yyyy-MM-ddTHH:mm:ss.fff"_1)
                    + ";" + 0.34325723854781f
                    + ";" + wiiDevice.WiimoteState.BalanceBoardState.CenterOfGravity.Y
                    + ";" + wiiDevice.WiimoteState.BalanceBoardState.SensorValuesKg.TopLeft
                    + ";" + wiiDevice.WiimoteState.BalanceBoardState.SensorValuesKg.TopRight
                    + ";" + wiiDevice.WiimoteState.BalanceBoardState.SensorValuesKg.BottomLeft
                    + ";" + wiiDevice.WiimoteState.BalanceBoardState.SensorValuesKg.BottomRight;
                File.AppendAllLines(recordingFile1, new string[] { data });
            }
            else
            {
                // Stop the recording.
